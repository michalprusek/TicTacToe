...
class GameController(QObject):
    ...
    def make_arm_move_with_symbol(self, symbol_to_play):
        ...
        # Get current board for strategy
        current_board_for_strategy = None
        if hasattr(self.main_window, 'board_widget') and self.main_window.board_widget:
            current_board_for_strategy = self.main_window.board_widget.board
        
        if not current_board_for_strategy:
            self.logger.error("Cannot get current board state for strategy.")
            return False
    ...
    def _check_game_end(self):
        ...
        board_to_check = None
        if hasattr(self.main_window, 'board_widget') and self.main_window.board_widget:
            board_to_check = self.main_window.board_widget.board
