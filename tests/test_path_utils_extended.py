...
class TestPathUtilsExtended:
    ...
    def test_get_project_root_contains_expected_files(self):
        ...
        expected_files = ['requirements.txt', 'pytest.ini', 'CLAUDE.md']
        found_files = [f for f in expected_files if os.path.exists(os.path.join(root, f))]
    ...
    def test_get_weights_path_returns_path(self):
        """Test that get_weights_path returns a string path."""
    ...
    def test_get_calibration_path_returns_path(self):
        """Test that get_calibration_path returns a string path."""
        cal_path = get_calibration_path()
        assert isinstance(cal_path, str)
        # Should contain 'calibration' in the path
        assert 'calibration' in cal_path
    ...
    def test_paths_are_absolute(self):
        ...
        assert os.path.isabs(calibration)
    ...
    def test_paths_consistency(self):
        ...
        weights_under_root = weights.startswith(root)
        assert weights_under_root
        
        # Calibration should be under project root
        calibration_under_root = calibration.startswith(root)
